...
const allCategories: ServiceCategory[] = [
  ...
  },
  {
    ...
    subKeys: [
      "sub_appliance_repair",
      "sub_computer_help",
      "sub_phone_repair",
      "sub_smart_home_setup",
    ],
    tab: "home",
  },
  {
...
export default function ServicesSection() {
  ...
  const filtered = useMemo(() => {
    return activeTab === "all" ? allCategories : allCategories.filter((c) => c.tab === activeTab);
  }, [activeTab]);
  ...
  return (
    <section className="py-24 bg-background">
      <div className="container px-4">
        ...
        {/* tabs (mobile: horizontal scroll, desktop: centered) */}
        <div className="mb-10 md:mb-12">
          <div className="-mx-4 px-4 overflow-x-auto no-scrollbar">
            <div className="inline-flex min-w-max md:w-full md:justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.slice(0, 8).map((category) => (
            <div
              key={category.id}
            ...
            >
              ...
              <ul className="space-y-2">
                {category.subKeys.slice(0, 4).map((subKey) => (
                  <li key={subKey}>
                    <Link
        ...
        <div className="text-center mt-12">
          <Link
            to="/categories"
            className="inline-block px-8 py-3 rounded-full bg-secondary hover:bg-secondary/80 text-secondary-foreground font-semibold transition-all duration-300 hover:-translate-y-0.5"
          >
            {t("services_view_all")} {allCategories.length}+{" "}
            {lang === "ru" ? "категорий" : lang === "ua" ? "категорій" : "categories"}
          </Link>
        </div>
      </div>
    </section>
